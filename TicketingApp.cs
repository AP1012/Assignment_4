...
public class TicketingApp
{
    ...
    {
        Console.WriteLine("\nPlease enter:");
        Console.WriteLine("1 to book a ticket.");
        Console.WriteLine("2 to see seating chart.");
        Console.WriteLine("3 to exit the application.");
    }
    ...
    private void BookTicket()
    {
        ...
        SeatLabel preferredSeat = SeatLabel.A; // Default to Window seat
        string input = Console.ReadLine();
        if (!string.IsNullOrEmpty(input) && int.TryParse(input, out int choice))
        {
            if (choice == 2)
            {
                preferredSeat = SeatLabel.B; // Aisle seat
            }
        }
